...
enum HelpContent {
    ...
    static let topics: [HelpTopic] = [
        ...
        HelpTopic(id: "trouble-cert", section: "Troubleshooting", title: "\"Certificate Changed\" — a host stopped responding after I replaced its certificate", symbol: "lock.shield",
            body: "NAS and Pi-hole boxes normally serve a certificate no public authority has signed, so there's nothing for Charopos to check it against. Instead it remembers the certificate each host presents the first time it connects, and refuses that host if it later presents a different one — the same trust-on-first-use idea as SSH host keys. This is what keeps the DSM and Pi-hole passwords from being handed to whatever happens to be answering at that address.\n\nA refused host is easy to spot: its light goes amber and its tooltip says the certificate was refused, and **Settings → Services → Certificates** marks it **REFUSED** in red, showing both the key it is presenting now and the key it was pinned to.\n\nIf you deliberately replaced that certificate (regenerated DSM's, moved to a real certificate, or rebuilt the box), click **Trust New Certificate** on that host's row. Charopos pins the key it is already presenting and resumes talking to it — including sending its password, which is why it asks first. **Forget** is the blunter alternative: it trusts whatever answers on the *next* connection, so prefer it only when the host will present something you haven't seen yet.\n\nIf you *didn't* change anything, don't clear it — a certificate that changes on its own is worth understanding first. The refusal keeps standing, and the host stays blocked, until you act on it.\n\nHosts with a genuinely trusted certificate (DSM with Let's Encrypt, say) are validated the ordinary way instead, so routine renewals never trigger this."),
